...
public class JwtFilterTest {
    ...
    void testDoFilterInternal_MissingAuthorizationHeader() throws ServletException, IOException {
        ...
        verify(auditService, times(1)).logAudit(eq(auditDTO), eq(HttpServletResponse.SC_UNAUTHORIZED), anyString(), isNull());
    ...
    void testDoFilterInternal_InvalidToken() throws JwtException, IllegalArgumentException, Exception {
        ...
        verify(auditService).logAudit(eq(auditDTO), eq(HttpServletResponse.SC_UNAUTHORIZED), eq("Invalid or expired JWT token"), anyString());
        assertEquals(HttpServletResponse.SC_UNAUTHORIZED, response.getStatus());
    }
}
